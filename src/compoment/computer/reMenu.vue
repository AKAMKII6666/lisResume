<template>
  <table class='menu-container' :style="{ marginTop:moveDetal.y+'px', marginLeft:moveDetal.x+'px'}" @click="hideContent()" >
      <tr>
          <td valign="middle" >
              <div class='title' >
                  {{this.title[lang]}}
              ...
              <div class='menus' >
                    <div  v-for='(item,index) in menuItem' :key='index' :style="{'textShadow':moveShadow.x+'px '+moveShadow.y+'px 10px rgba(0,0,0,0.8)'}"  :class='[item.selected ? "selected" : ""]'  >
                        <div class="totalCover" @mouseover="menuOver(item)"  @click="switchContent($event,item)" ></div>
