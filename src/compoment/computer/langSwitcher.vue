<template>
  <div class='container' >
      <div class='langChoose' v-for='(item,index) in langList' :key='index' > <span v-if="index != 0" >|</span> <label @click='switchLang(item)' >{{ item.name }}</label></div>
      <div class='preforList' v-if="!isLowLevelBrowser" >
          <span>{{ pText[lang] }}</span>
          <label  v-for='(item,index) in preforList' :key='index' ><span v-if="index != 0" >|</span> <label @click='switchPrf(item)' >{{ item.text[lang] }}</label></label>
      </div>
...
export default {
    ...
    data(){
        return {
            ...
            pLowText:['请更换浏览器为Chrome或者Edge使3D场景可见。',"Pls Change Your Borswer To Chrome OR Edge For View The 3D Scene."],
            preforList:[
                {
                    name:'low',
                    text:['低','Low']
