	...
	this.createButtons = function () {
		if (self.config.buttons.mode !== "none" && self.config.buttons.mode !== "free") {
			...
			if (self.config.buttons.mode === "yes") {
				self.buttonYes = $("<button class='b-w-b n-button' >确&nbsp;定</button>").appendTo(
			...
			if (self.config.buttons.mode === "yesno") {
				self.buttonCancle = $(
					"<button class='b-w-b n-button'  >取&nbsp;消</button>&nbsp;&nbsp;"
		...
		if (self.config.buttons.mode === "free") {
			...
			for (var i = 0; i < self.config.buttons.arr.length; i++) {
				...
				var elem = $(
					"<button class='b-w-b n-button'  >" + item.name + "</button>&nbsp;&nbsp;"
				).appendTo(self.buttonContainer);
		...
		if (self.config.position.x === "tips") {
			...
			if (self.screenState === "V") {
				...
				if (self.content.height() > $(window).height()) {
					...
					self.content.animate(
						{ top: $(window).height() - self.content.height(), opacity: 1 },
						350
					);
				}
			} else {
